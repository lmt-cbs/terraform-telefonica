# Creacion del recurso Lambda que coresponde con la llamada a getDiscount.
#
# el codigo va en zip y se almacena en la carpeta de del bucket s3 creado

resource "aws_lambda_function" "getDisc" {
  function_name = "tf-${terraform.workspace}-${var.lambda_getDisc}-${random_id.lambda_getDisc_rand.hex}"
