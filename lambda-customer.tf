# Creacion del recurso Lambda que coresponde con la llamada a customer.
#
# el codigo va en zip y se almacena en la carpeta de del bucket s3 creado

resource "aws_lambda_function" "customer" {
  function_name = "tf-${terraform.workspace}-${var.lambda_customer}-${random_id.lambda_customer_rand.hex}"
